...
public class Task3 {
    ...
    private static void getPositiveAndNegativeNumbersOfArray(int[] result) {
        ...
        for (int i = 0; i < result.length; i++) {
            if (result[i] < 0) {
                int[] arrayNegativeNumbers = Arrays.copyOf(result, 200_006);
                System.out.print(arrayNegativeNumbers[i] + " ");
            } else {
                int[] arrayPositiveNumbers = Arrays.copyOf(result, 200_000);
                System.out.print(arrayPositiveNumbers[i] + " ");
            }
        }
    }
